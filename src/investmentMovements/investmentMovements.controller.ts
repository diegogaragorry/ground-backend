...
function parseBody(body: any) {
  ...
  if (!["deposit", "withdrawal", "yield"].includes(type)) return null;
  if (!currencyId) return null;
  if (!hasEncrypted && (!Number.isFinite(amount) || amount < 0)) return null;
...
export const listInvestmentMovements = async (req: AuthRequest, res: Response) => {
...
export const createInvestmentMovement = async (req: AuthRequest, res: Response) => {
  const userId = req.userId!;
  ...
  if (!inv) return res.status(403).json({ error: "Invalid investmentId for this user" });

  const row = await prisma.investmentMovement.create({
    data: {
...
export const updateInvestmentMovement = async (req: AuthRequest, res: Response) => {
  const userId = req.userId!;
  ...
  const existing = await prisma.investmentMovement.findFirst({
  ...
  const row = await prisma.investmentMovement.update({
    where: { id },
    data: {
      investmentId: parsed.investmentId,
      date: parsed.date,
...
export const deleteInvestmentMovement = async (req: AuthRequest, res: Response) => {
  const userId = req.userId!;
  const id = String(req.params.id ?? "");
  if (!id) return res.status(400).json({ error: "id is required" });
  ...
  await prisma.investmentMovement.delete({ where: { id } });
  res.status(204).send();
};