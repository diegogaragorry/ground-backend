  ...
  const missingRows = monthsOpen.flatMap((month) =>
    templates
      ...
      .map((template) => ({
        ...
        reminderLabel: (template as any).reminderLabel ?? null,
...
export const updatePlannedExpense = async (req: AuthRequest, res: Response) => {
  ...
  if (hasEncrypted) {
    ...
    patch.description = encryptedPlaceholder();
    patch.reminderLabel = parseReminderLabel(req.body?.reminderLabel, (pe as any).reminderLabel ?? null);
    patch.amountUsd = 0;
    patch.amount = 0;
  } else {
  const isUyu = pe.template?.defaultCurrencyId === "UYU";
    ...
    try {
      const result = await prisma.$transaction(async (tx) => {
        let working: PlannedExpenseForConfirm = {
          ...
          reminderLabel: (base as any).reminderLabel ?? null,
