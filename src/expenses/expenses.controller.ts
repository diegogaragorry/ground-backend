...
function parseBatchAmount(value: unknown) {
  const n = Number(value);
  return Number.isFinite(n) ? n : null;
}

function parseLearnedRuleInput(raw: any, index: number) {
  const merchantFingerprint = typeof raw?.merchantFingerprint === "string" ? raw.merchantFingerprint.trim() : "";
  if (!merchantFingerprint) throw new Error(`Rule ${index + 1}: merchantFingerprint is required`);

  const categoryId = typeof raw?.categoryId === "string" ? raw.categoryId.trim() : "";
  if (!categoryId) throw new Error(`Rule ${index + 1}: categoryId is required`);

  const encryptedPayload =
    typeof raw?.encryptedPayload === "string" ? raw.encryptedPayload.trim() : "";
  if (!encryptedPayload) throw new Error(`Rule ${index + 1}: encryptedPayload is required`);

  const expenseType =
    raw?.expenseType === "FIXED" || raw?.expenseType === "VARIABLE"
      ? raw.expenseType
      : null;
...
export const createExpense = async (req: AuthRequest, res: Response) => {
  ...
  if (hasEncrypted) {
    ...
    try {
      fx = toUsd({ amount: amt, currencyId, usdUyuRate });
    } catch (e: any) {
...
export const importExpensesBatch = async (req: AuthRequest, res: Response) => {
  const userId = req.userId!;
  ...
  const normalizedItems = rawItems.map((raw, index) => {
    ...
    if (!categoryId) throw new Error(`Row ${index + 1}: categoryId is required`);

    const currencyId = typeof raw?.currencyId === "string" ? raw.currencyId : "";
    if (!currencyId) throw new Error(`Row ${index + 1}: currencyId is required`);
  ...
  const categoryIds = [...new Set<string>(normalizedItems.map((item) => item.categoryId))];
  for (const rule of normalizedLearnedRules) {
    categoryIds.push(rule.categoryId);
  }
  const currencyIds = [...new Set<string>(normalizedItems.map((item) => item.currencyId))];
  const years = [...new Set<number>(normalizedItems.map((item) => item.date.getUTCFullYear()))];
  ...
  const dedupedLearnedRules = new Map<string, (typeof normalizedLearnedRules)[number]>();
  for (const rule of normalizedLearnedRules) {
...
export const listMerchantMappingRules = async (req: AuthRequest, res: Response) => {
  const userId = req.userId!;

  const rows = await prisma.merchantMappingRule.findMany({
    where: { userId },
...
export const updateExpense = async (req: AuthRequest, res: Response) => {
  ...
  if (updatingWithEncrypted) {
    data.amountUsd = 0;
    if (usdUyuRate !== undefined) data.usdUyuRate = usdUyuRate;
  } else if (amount !== undefined || currencyId !== undefined || usdUyuRate !== undefined) {
    const finalAmount = amount !== undefined ? (hasEncryptedPayload ? 0 : amount) : (existing as any).amount;
