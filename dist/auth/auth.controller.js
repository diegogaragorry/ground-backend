...
exports.me = exports.login = exports.registerVerify = exports.registerRequestCode = void 0;
...
const registerRequestCode = async (req, res) => {
    ...
    const rec = await prisma_1.prisma.emailVerificationCode.create({
        data: {
            email,
            codeHash,
            purpose,
    ...
    try {
        await (0, mailer_1.sendSignupCodeEmail)(email, code);
    }
    catch (err) {
        console.error("sendSignupCodeEmail error:", err);
        // Limpieza (para no dejar códigos muertos)
        try {
            await prisma_1.prisma.emailVerificationCode.delete({ where: { id: rec.id } });
        }
        catch {
            // ignore
        }
