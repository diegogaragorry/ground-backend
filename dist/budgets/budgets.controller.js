...
const listBudgets = async (req, res) => {
    ...
    if (!ym)
        return res.status(400).json({ error: "Provide year and month query params (e.g., ?year=2026&month=1)" });
...
const budgetReport = async (req, res) => {
    const userId = req.userId;
    const ym = parseYearMonth(req.query);
    if (!ym)
        return res.status(400).json({ error: "Provide year and month query params (e.g., ?year=2026&month=1)" });
    ...
    const rows = budgets.map((b) => {
        ...
        return {
            ...
            diff,
            pct,
            encryptedPayload: b.encryptedPayload ?? undefined,
        };
    });
...
========================================================= */
const updateOtherExpenses = async (req, res) => {
    const userId = req.userId;
    ...
    const row = await prisma_1.prisma.monthlyBudget.upsert({
...
exports.updateOtherExpenses = updateOtherExpenses;
...
async function buildAnnualData(userId, year) {
    ...
    for (const c of closes)
        closeByMonth.set(c.month, c);
    // -------- Income (USD) per month
    const incomeRows = await prisma_1.prisma.income.findMany({
        where: { userId, year },
        select: { month: true, amountUsd: true },
    });
    const incomeByMonth = new Map();
    for (const r of incomeRows)
        incomeByMonth.set(r.month, r.amountUsd ?? 0);
    // -------- Actual expenses (USD) per month from Expenses
    const yearStart = new Date(Date.UTC(year, 0, 1, 0, 0, 0));
    const yearEnd = new Date(Date.UTC(year + 1, 0, 1, 0, 0, 0));
    const expensesRows = await prisma_1.prisma.expense.findMany({
    ...
    const otherRows = await prisma_1.prisma.monthlyBudget.findMany({
        where: { userId, year },
    ...
    function capitalUsdPortfolio(inv, byM, m) {
        ...
        if (baseMonth == null || baseUsd == null)
            return 0;
        const yStart = yieldStartMonthForYear(inv, year);
    ...
    function capitalUsdAccountCarry(inv, byM, m) {
        const directSnap = byM.get(m);
        if (directSnap) {
            const directUsd = snapUsdFor(inv, directSnap, m);
            if (directUsd != null)
    ...
    const portfolioNW = months12.map((m) => portfolios.reduce((acc, inv) => {
    ...
    const projectedNextJanPortfolio = portfolios.reduce((acc, inv) => {
        const byM = buildInvMonthMap(inv.id, snaps);
        const decCap = capitalUsdPortfolio(inv, byM, 12);
    ...
    const portfolioVariationShift = months12.map((m, idx) => {
        if (m < 12)
            return (portfolioNW[idx + 1] ?? 0) - (portfolioNW[idx] ?? 0);
        return projectedNextJanPortfolio - (portfolioNW[11] ?? 0);
    });
    // -------- Flows by month (Portfolio only, USD + UYU converted to USD)
    const movementRows = await prisma_1.prisma.investmentMovement.findMany({
        where: {
    ...
    const accountsNW = months12.map((m) => accounts.reduce((acc, inv) => {
        const byM = buildInvMonthMap(inv.id, snaps);
        return acc + capitalUsdAccountCarry(inv, byM, m);
    }, 0));
    ...
    for (let idx = 0; idx < 12; idx++) {
        ...
        if (locked) {
            ...
            const lockedEncryptedPayload = locked.encryptedPayload ?? undefined;
            const isClosed = locked.isClosed !== false;
            months.push({
                month: m,
                isClosed,
